...
static uint8_t I2C_read(uint8_t reg_addr) {
	...
	while ((status = !I2C_GetFlagStatus(I2C1, I2C_FLAG_SB)) && !timer_elapsed(I2C_TIMEOUT))
		;
	if (status) {
	...
	while ((status=!I2C_CheckEvent(I2C1, I2C_EVENT_MASTER_BYTE_TRANSMITTED)) && !timer_elapsed(I2C_TIMEOUT))
		;
	if (status) {
    ...
    while ((status = !I2C_CheckEvent(I2C1, I2C_EVENT_MASTER_BYTE_RECEIVED)) && !timer_elapsed(I2C_TIMEOUT));
...
if (status) {
	...
	/*stop bit flag*/
	while ((status = I2C_GetFlagStatus(I2C1, I2C_FLAG_STOPF)) && !timer_elapsed(I2C_TIMEOUT))
		;
	if (status) {
